...
void DDR3::init_prereq()
{
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<DDR3>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return Command::ACT_R;
    ...
    // WR
    prereq[int(Level::Rank)][int(Command::WR)] = [] (DRAM<DDR3>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::PowerUp): return Command::MAX;
            case int(State::ActPowerDown): return Command::PDX;
            case int(State::PrePowerDown): return Command::PDX;
    ...
    prereq[int(Level::Bank)][int(Command::WR)] = [] (DRAM<DDR3>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return Command::ACT_W;
            case int(State::Opened):
                if (node->row_state.find(id) != node->row_state.end())
                    return cmd;
                return Command::PRE;
...
{
    lambda[int(Level::Bank)][int(Command::ACT_R)] = [] (DRAM<DDR3>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::ACT_W)] = [] (DRAM<DDR3>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};    
...
void DDR3::init_timing()
{
    ...
    //Added by MG
    // t[int(Command::WR)].push_back({Command::WR, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WR)].push_back({Command::WRA, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WRA)].push_back({Command::WR, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WRA)].push_back({Command::WRA, 1, s.nBL + s.nRTRS, true});    
    // t[int(Command::WR)].push_back({Command::RD, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WR)].push_back({Command::RDA, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WRA)].push_back({Command::RD, 1, s.nBL + s.nRTRS, true});
    // t[int(Command::WRA)].push_back({Command::RDA, 1, s.nBL + s.nRTRS, true});
    ...
    // RAS <-> RAS
    t[int(Command::ACT_R)].push_back({Command::ACT_R, 1, s.nRRD});
    t[int(Command::ACT_W)].push_back({Command::ACT_W, 1, s.nRRD});
    t[int(Command::ACT_R)].push_back({Command::ACT_W, 1, s.nRRD});
    t[int(Command::ACT_W)].push_back({Command::ACT_R, 1, s.nRRD});
    t[int(Command::ACT_R)].push_back({Command::ACT_W, 4, s.nFAW});
    t[int(Command::ACT_R)].push_back({Command::ACT_R, 4, s.nFAW});
    t[int(Command::ACT_W)].push_back({Command::ACT_W, 4, s.nFAW});
    t[int(Command::ACT_W)].push_back({Command::ACT_R, 4, s.nFAW});
    t[int(Command::ACT_R)].push_back({Command::PREA, 1, s.nRAS});
    t[int(Command::ACT_W)].push_back({Command::PREA, 1, s.nRAS});
    t[int(Command::PREA)].push_back({Command::ACT_R, 1, s.nRP});
    t[int(Command::PREA)].push_back({Command::ACT_W, 1, s.nRP});

    // RAS <-> REF
    t[int(Command::PRE)].push_back({Command::REF, 1, s.nRP});
    t[int(Command::PREA)].push_back({Command::REF, 1, s.nRP});
    t[int(Command::REF)].push_back({Command::ACT_R, 1, s.nRFC});
    t[int(Command::REF)].push_back({Command::ACT_W, 1, s.nRFC});

    // RAS <-> PD
    t[int(Command::ACT_R)].push_back({Command::PDE, 1, 1});
    t[int(Command::ACT_W)].push_back({Command::PDE, 1, 1});
    t[int(Command::PDX)].push_back({Command::ACT_R, 1, s.nXP});
    t[int(Command::PDX)].push_back({Command::ACT_W, 1, s.nXP});
    ...
    t[int(Command::SRX)].push_back({Command::ACT_R, 1, s.nXS});
    t[int(Command::SRX)].push_back({Command::ACT_W, 1, s.nXS});
    ...
    // CAS <-> RAS
    t[int(Command::ACT_R)].push_back({Command::RD, 1, s.nRCD});
    t[int(Command::ACT_W)].push_back({Command::RD, 1, s.nRCD});
    t[int(Command::ACT_R)].push_back({Command::RDA, 1, s.nRCD});
    t[int(Command::ACT_W)].push_back({Command::RDA, 1, s.nRCD});
    t[int(Command::ACT_R)].push_back({Command::WR, 1, s.nRCD});
    t[int(Command::ACT_W)].push_back({Command::WR, 1, s.nRCD});
    t[int(Command::ACT_R)].push_back({Command::WRA, 1, s.nRCD});
    t[int(Command::ACT_W)].push_back({Command::WRA, 1, s.nRCD});
    t[int(Command::RD)].push_back({Command::PRE, 1, s.nRTP});
    t[int(Command::WR)].push_back({Command::PRE, 1, s.nCWL + s.nBL + s.nWR});
    t[int(Command::RDA)].push_back({Command::ACT_R, 1, s.nRTP + s.nRP});
    t[int(Command::RDA)].push_back({Command::ACT_W, 1, s.nRTP + s.nRP});
    t[int(Command::WRA)].push_back({Command::ACT_R, 1, s.nCWL + s.nBL + s.nWR + s.nRP});
    t[int(Command::WRA)].push_back({Command::ACT_W, 1, s.nCWL + s.nBL + s.nWR + s.nRP});

    // RAS <-> RAS
    t[int(Command::ACT_R)].push_back({Command::ACT_R, 1, s.nRC});
    t[int(Command::ACT_R)].push_back({Command::ACT_W, 1, s.nRC});
    t[int(Command::ACT_W)].push_back({Command::ACT_W, 1, s.nRC});
    t[int(Command::ACT_W)].push_back({Command::ACT_R, 1, s.nRC});
    t[int(Command::ACT_R)].push_back({Command::PRE, 1, s.nRAS});
    t[int(Command::ACT_W)].push_back({Command::PRE, 1, s.nRAS});
    t[int(Command::PRE)].push_back({Command::ACT_R, 1, s.nRP});
    t[int(Command::PRE)].push_back({Command::ACT_W, 1, s.nRP});
}